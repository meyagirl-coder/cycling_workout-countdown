...
**踩過的坑：`ctx.resume()` 的 Promise resolve 了，但 `AudioContext` 實際上
還是沒有真的恢復播放**——使用者實測回報「用開團連結加入團練、同時開
Google Meet 視訊會議分享畫面」這個組合下，**只有課表第一組的嗶聲有聲音，
之後每一組都完全沒聲音**。追查後這不是「每次都會 suspend、每次都要
resume()」這個已知機制本身的問題（那個機制在一般情境下運作正常，第一組
本來就靠它才有聲音）——而是分頁被分享畫面／切到視訊軟體之後長時間背景化，
`ctx.resume()` 這個 Promise 有時會確實 resolve，但底下的 `AudioContext`
實際上並沒有真的把 `state` 變回 `"running"`（部分瀏覽器在「分頁音訊被
分頁擷取」＋「長時間背景化」這兩個條件疊加時的已知不一致行為）。原本的
程式碼 resolve 之後就直接排程音效節點、完全沒有再檢查 `ctx.state`，一旦
遇到這個狀況，`sharedAudioContext` 這個模組層級變數會一直卡在同一個「看似
resume 完成、實際上還是壞掉」的物件上——不只這一組沒聲音，之後每一組都會
重複同一個失敗，永遠恢復不了（跟使用者回報的「只有第一組有聲音」完全對得
上）。修法（`playCountdownBeeps()`）：`resume()` 的 `.then()` 裡多一層
檢查，resolve 之後如果 `ctx.state` 還不是 `"running"`（或 `resume()`
本身直接 reject），就整個丟棄這個壞掉的 `AudioContext`、重新 `new` 一個
乾淨的實例來排這一組的音效，讓「這一組壞掉」不會拖累後面每一組——下一次
呼叫拿到的是全新、獨立的一次機會，不會一直卡在同一個壞掉的物件上。
...
- **輸入格式**：文字輸入框（不是瀏覽器原生的日期選擇器），格式是「年月日
  ...
  - **全形數字容錯**（regression：使用者回報「產生開團分享連結」工具明明
    輸入了合法的 12 位數字卻判定格式錯誤）：中文／日文輸入法切到全形模式
    時，就算欄位設了 `inputmode="numeric"`，打出來的數字字元還是可能是
    全形（０-９，U+FF10–FF19）——在小小的輸入框裡肉眼幾乎看不出跟半形數字
    的差異，`\d` 卻只吃半形數字。`parseScheduledStartTimeInput()` 會先把
    全形數字轉成半形再驗證格式，兩個輸入欄位（「設定開始時間」／「產生
    開團分享連結」）共用同一個函式，這個容錯對兩邊都生效。
...
- **設定當下解鎖自動播放權限**：使用者按下「設定」按鈕的當下（同一個 click
  ...
  提示音效／語音才不會被瀏覽器擋掉。
- **開始時間已過去 → 跳到當下對應的進度直接播放（不是從頭開始）**：
  `armSchedule()` 比對設定的時間戳跟 `Date.now()`，已經過去就呼叫
  `startScheduledWorkoutNow(workout, startTimestamp)`：
  - 算出 `elapsedSeconds = (Date.now() - startTimestamp) / 1000`（已經過去
    幾秒），用 `switchToPlayerScreen(workout, { elapsedTotal: elapsedSeconds,
    powerAdjustPct: 0, status: 'paused' })` 透過 `client.restore()` 把引擎
    狀態直接設到這個累計時間點對應的組別／組內剩餘秒數，緊接著呼叫
    `client.play()` 從這個位置開始即時往前跑——不是從第一組、0 分鐘重新
    開始，也不是停在那個時間點暫停等使用者自己按播放，而是動態進行中，
    讓遲到的使用者能立刻跟上團練當下真正在跑的組別（regression：使用者
    實測遲到後點開開團連結，畫面卻從頭播放，完全跟不上）。
  - 技術上這是借用 `client.restore()`（原本是給「重新整理頁面復原進度」
    用，見下面「頁面狀態保存」的說明，設計上永遠不會把狀態設回
    `running`）先把 `elapsedTotal` 設到正確位置，再緊接著呼叫
    `client.play()`（用「現在時間 - elapsedTotal」反推
    `startTimestamp`，之後 `tick()` 自然從這個位置即時往前推進）達成「跳到
    對應組別＋動態進行中」的效果，不需要額外的引擎 API。
  - **超過總時長時顯示「課表已結束」**：如果 `elapsedSeconds` 已經 >= 這份
    課表的 `totalDuration`（例如課表只有 30 分鐘但已經過去 40 分鐘），
    沒有一個「對應的組別」可以跳，不會嘗試切到執行頁，改成用
    `uploadView.showError()` 顯示「課表已結束：設定的開始時間已經過去
    X，超過這份課表 Y 的總時長，請重新設定開始時間，或直接手動開始
    播放。」，停留在目前畫面。
  - 不管是「已過去、直接開始」還是「還沒到、進等待畫面倒數後時間到自動
    開始」，都不經過中繼的暫停狀態——使用者看到的畫面永遠是動態進行中。
- **開始時間還沒到 → 一頁式等待（不是獨立的等待畫面）**：`armSchedule()`
  呼叫 `enterOnePageWaitingPhase()`（`playerApp.js`），同時做兩件事，兩者
  疊在同一頁、不是切到另一個畫面：
  1. 先呼叫 `client.init(workout)` 把底下的執行頁（`playerMount`）渲染成
     「尚未開始」的預覽狀態——時間軸／課表名稱／總時長／組數／下一組資訊
     都跟使用者一般點開課表、還沒按開始播放時看到的畫面一致（`倒數計時`
     顯示第一組的時長，`.interval-progress` 顯示「尚未開始」）。
  2. 在執行頁上方疊加顯示等待倒數橫幅（`groupJoinConfirmView.js` 的等待
     階段，`.group-join-waiting-phase`——雖然檔名跟大部分命名還留著「開團
     連結」這個歷史名稱，但這個橫幅現在是「設定開始時間」手動排程／開團
     連結／開機復原排程三條路徑共用的元件，確保三種進入方式看到完全一樣
     的等待呈現），大字倒數「距離開始還有 X天Y小時Z分W秒」——規格要求精確
     到秒、每秒即時更新（不是只精確到分鐘）。用中文單位逐級顯示，較大的
     單位出現時後面所有較小的單位都要一併顯示（就算是 0，不會跳過中間
     單位）：`距離開始還有 45秒` → `距離開始還有 3分10秒` → `距離開始還有
     3小時2分10秒` → `距離開始還有 2天3小時2分10秒`（`formatRemainingLabel()`，
     見 `scheduledStartRuntime.js`）。由 `scheduledStartRuntime.js` 的
     `createScheduledStartRuntime()` 每秒重新計算一次「還剩多少毫秒」（不是
     用累計 tick 次數推算，背景分頁被降頻、某次 tick 晚到時，下一次 tick
     仍然會自我修正成正確的剩餘時間，跟 `timerEngine.js` 對抗計時漂移的做法
     一致）。橫幅上有「取消排程」按鈕，可以中止排程回到上傳畫面。
  - **時間到時的轉場**：只把等待倒數橫幅收起來（`groupJoinConfirmMount`
    加上 `.hidden`），並呼叫 `startScheduledWorkoutNow()` 把底下已經渲染好
    的執行頁從「尚未開始」切成「進行中」（完整的計時／切組／倒數提示邏輯
    都適用，跟手動點開始沒有差別）——不整頁重新渲染、不跳轉，執行頁本身
    從頭到尾就是同一份 DOM，只是狀態從 idle 換成 running，視覺上沒有跳動
    感。這跟開團連結按下「加入團練」之後的轉場是同一套機制。
- **限制提醒**：等待倒數橫幅上有明顯的提醒文字，說明如果分頁被完全關閉、
  或裝置長時間背景休眠導致系統回收資源，「自動開始」可能會失效，建議使用者
  在排定時間前至少重新打開一次分頁確認狀態。
...
- **App 開機時的處理流程**（`playerApp.js` 的 `handleGroupJoinLinkBoot()`）：
  ...
  2. FTP 確定後，`startGroupJoinFlow()` 把 `startTime` 存進
     `pendingGroupJoinStartTimestamp`，並依 `source` 呼叫對應的課表下載流程
     （`source=TD` 呼叫既有的 `handleTrainerDayUrlSubmit()`）。
  3. 課表下載/解析成功後，**不會**直接套用排程（跳等待畫面或直接播放），
     而是先切到「加入確認」畫面（`groupJoinConfirmView.js`）：顯示課表
     名稱／總時長／組數、排定開始時間，跟一顆「加入團練」按鈕。
     **這一步是刻意的，取代了早期「頁面載入時直接嘗試解鎖自動播放權限」的
     做法**——那個做法在真機（iOS Safari／Chrome）實測後確認不可靠、仍然
     完全無聲：這條流程是頁面載入當下自動觸發的，沒有使用者互動的呼叫
     堆疊，瀏覽器的自動播放權限解鎖（`unlockAudioAndSpeechForAutoplay()`）
     嚴格要求「使用者互動當下」才保證有效。改成先顯示確認畫面，使用者按下
     「加入團練」的那個點擊，才是真正的使用者互動——`handleGroupJoinConfirm()`
     在這個 click handler 裡同步呼叫解鎖，跟「設定開始時間」按鈕用的是
     同一套機制，才能保證後續自動觸發的語音／嗶聲穩定運作。
  4. 按下「加入團練」之後才呼叫 `saveSchedule()` + `armSchedule()`——完全
     沿用「手動貼網址前先按過『設定開始時間』」的既有排程判斷：`startTime`
     已經過去就跳到對應的進度直接播放，還沒到就進等待畫面倒數（遲到加入
     的使用者會直接跟上團練當下的組別／秒數，不是從頭開始，詳見上面 5.3
     節「開始時間已過去」的說明），不用另外重新實作一次排程判斷。
  5. 這個確認畫面只在「透過開團連結進入」的情境出現（不管是真的點擊分享
     連結、還是使用者自己手動在網址列輸入完整網址——只要偵測到網址帶有
     `source`/`source_url`/`startTime` 參數就會出現），其他 4 種輸入方式
     （貼文字／貼網址／上傳 .zwo／intervals.icu）維持原本一鍵直接載入的
     操作流程，不受影響；開機時從 `scheduleStore.js` 復原「已經確認過」的
     排程（例如重新整理頁面）也不會重新出現這個確認畫面。
