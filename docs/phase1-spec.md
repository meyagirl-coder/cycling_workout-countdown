...
- **App 開機時的處理流程**（`playerApp.js` 的 `handleGroupJoinLinkBoot()`）：
  ...
  2. FTP 確定後，`startGroupJoinFlow()` 把 `startTime` 存進
     ...
     `startTime` 已經過去就跳到對應的進度直接播放，還沒到就進等待畫面倒數
     ——遲到加入的使用者會直接跟上團練當下的組別／秒數，不是從頭開始，
     詳見上面 5.3 節「開始時間已過去」的說明），不用另外重新實作一次排程
     判斷。
  3. **已知限制**：這個流程是頁面載入當下自動觸發，不是使用者按鈕點擊的
     當下，嚴格來說瀏覽器的自動播放權限解鎖（`unlockAudioAndSpeechForAutoplay()`）
     需要「使用者互動當下」的呼叫堆疊才保證有效，這裡沒有那個時機。仍然
     選擇呼叫這裡（不額外插入「點擊以加入」的確認畫面）：部分瀏覽器對
     「這個網域使用者之前互動過」有更寬鬆的自動播放判斷，呼叫了至少有機會
     解鎖成功；呼叫不到位、被瀏覽器擋掉時，倒數提示的語音／嗶聲可能要等
     使用者在頁面上做過一次真正的互動（例如點擊任何按鈕）之後才會正常
     播放——這是瀏覽器自動播放政策的既有限制，不是每個瀏覽器都保證吃這
     一套。
