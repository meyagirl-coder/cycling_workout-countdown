...
接近真實頁面的內容，一次補齊三個 §3.7 原本沒處理的格式細節：

1. **行首狀態標籤**（`Active`／`Rest`／`Cooldown`／`Warmup` 等）：純描述性
   文字，`TRAINERDAY_STRUCTURE_LINE_RE` 現在允許行首出現 0 個以上這種標籤
   單字，不影響後面時長／百分比的判斷。
...
1. **優先判斷 §3.6 格式**：只要整份文字裡出現「持续时間」總時長說明行，或
   是整行寫完的重複組 `NX (...)`，就直接判定是 §3.6 的格式——這兩種寫法是
   §3.6 獨有的，不會出現在其他格式裡，優先於其他格式的判斷。
