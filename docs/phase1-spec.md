...
- **開始時間還沒到 → 等待畫面**：`waitingView.js` 顯示課表名稱／總時長／
  組數，加上大字倒數「距離開始還有 mm:ss」——規格要求精確到分秒、每秒即時
  更新（不是只精確到分鐘），格式重用 `formatMMSS()`（執行頁本組倒數計時
  用的同一個函式），讓等待畫面跟執行頁看到的是同一種「mm:ss」視覺語言。
  剩餘時間超過 1 小時／1 天時，在 mm:ss 前面加上「X 小時」／「X 天 Y 小時」
  （mm:ss 只表示這一小時內還剩多少分秒，分鐘部分歸零重算，不會出現
  「125:32」這種超過 60 的分鐘數）：`距離開始還有 5:32` → `距離開始還有
  3 小時 5:32` → `距離開始還有 2 天 3 小時 5:32`（`formatRemainingLabel()`，
