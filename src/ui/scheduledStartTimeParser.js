/**
 * 「設定開始時間」欄位的文字格式解析：`YYYYMMDD HH:mm`（年月日連續 8 位數字
 * ＋一個空格＋24 小時制時間），例如 `20260724 20:00`。純函式，不碰 DOM，
 * 方便單獨測試；用 `new Date(year, monthIndex, day, hour, minute)` 建構，
 * 這個建構子用的是瀏覽器所在裝置的當地時區，不是 UTC，符合「用使用者裝置
 * 當地時區」的規格要求，不需要額外時區換算。
 *
 * 格式錯誤（缺空格、位數不對、超出合法範圍、月份沒有那一天等）一律拋出
 * 清楚指出問題的 Error，不做靜默失敗或猜測使用者的意圖。
 */
const FORMAT_RE = /^(\d{4})(\d{2})(\d{2})\s+(\d{2}):(\d{2})$/;

const FORMAT_ERROR_MESSAGE = '日期時間格式錯誤，請用「20260724 20:00」這種格式（年月日連續 8 位數字 + 空格 + 24 小時制時間）';

/**
 * @param {string} text - 使用者輸入的文字，例如 "20260724 20:00"
 * @returns {Date}
 */
