...
// 太快、太短促，數字跟數字之間幾乎沒有間隔，一度調到 0.9（正好是 1.8 的一半）
// ——但實測（多種輸入方式 × PC/iOS）發現放到 0.9 這麼慢之後，配合下面
// speakCountdownWarning() 的 cancel()-再-speak() 安全網，真機的 TTS 引擎在
// 快速「打斷上一句、講新一句」時常出現開頭被吃掉、甚至整句被靜默丟棄的情況
// （這是 Chromium/WebKit 已知的行為，不是這裡邏輯錯誤——cancel() 後緊接著
// speak()，在真實裝置上不保證新的這句能完整、穩定地開始）。這裡回調到 1.3，
// 比原本的 1.8 慢、但沒有 0.9 那麼慢，降低單一數字講超過 1 秒、需要頻繁靠
// cancel() 打斷上一句的機率，同時仍然比最早的 1.8 更從容。跟上面同樣的限制：
// 瀏覽器不保證實際唸完要多久，這個值是盡力而為的估計值，不是精確保證——
// 如果實測後這台裝置還是覺得太趕或太慢，只要調整這個常數即可，報數本身的
// 觸發時機不受影響（見上方對應的模組說明）。
//
// 沿革：一開始用預設語速（rate=1）唸完單一個數字實際花的時間比畫面上的 1
// 秒還久，調快到 1.8 讓它趕上畫面倒數的節奏；後來使用者回報這個語速反而
// 太快太短暫，調到 0.9；真機測試發現 0.9 配合 cancel() 會讓報數不穩定（開頭
// 被吃掉／整句消失），才回調到這裡的 1.3。speakCountdownWarning() 仍然保留
// 每次講新的一句前先呼叫 speechSynthesis.cancel() 的安全網（見該函式的
// 說明），這裡回調語速是為了降低真的需要靠這個安全網介入的頻率。
