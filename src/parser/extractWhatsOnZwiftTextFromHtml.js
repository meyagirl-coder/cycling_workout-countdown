 ...
 * （ramp／steady／複合重複組），組成一份可以直接餵給 parseWhatsOnZwiftText()
 * 的純文字——「什麼樣的行算課表內容」跟 whatsOnZwiftParser.js 共用同一套
 * 正則，不重寫一份新的判斷邏輯。
 *
 ...
 * 一個空行（複合重複組雖然是單行格式、不像 TrainerDay 格式需要靠空行判斷
 * 「Nx」結束在哪，但保留段落間隔對可讀性／除錯還是有幫助，也讓兩個擷取模組
 * 的邏輯維持一致）。
 *
 * 跟 extractWorkoutTextFromHtml.js（TrainerDay）不同，這裡刻意不做「寬鬆
 * 模式」的片段搜尋備援——WhatsOnZwift 的複合重複組格式（同一行逗號分隔兩段）
 * 比 TrainerDay 單純的「X min @ Yw」複雜很多，如果不要求整行完全符合格式、
 * 只在一大段文字裡搜尋片段，很容易把不相關的內容誤組成一個「看起來合理但
 * 其實是錯的」複合重複組——這種「安靜地算錯」比直接擷取失敗、提示使用者
 * 改用「直接複製貼上文字內容」更危險，所以沒有比對到任何整行格式就直接視為
 * 擷取失敗。
 */
import { WOZ_RAMP_LINE_RE, WOZ_REPEAT_LINE_RE, WOZ_STEADY_LINE_RE } from './whatsOnZwiftParser.js';
...
export function extractWhatsOnZwiftTextFromHtml(html) {
  ...
  const matchedLines = collapseToMatchingLines(
    lines,
    (line) => WOZ_REPEAT_LINE_RE.test(line) || WOZ_RAMP_LINE_RE.test(line) || WOZ_STEADY_LINE_RE.test(line)
  );
