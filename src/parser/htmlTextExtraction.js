 ...
 * （extractWorkoutTextFromHtml.js / extractWhatsOnZwiftTextFromHtml.js）共用
 * ——「怎麼把 HTML 轉成一行一行的文字」跟「哪些行算課表內容」是兩件事，這裡
 * 只處理前者，後者由各自的 parser 提供判斷用的正則。
 */
 ...
 * 課表文字 parser（parsePasteText／parseWhatsOnZwiftText）靠空行判斷「Nx」
 * 重複組在哪裡結束，這裡如果把中間的雜訊直接砍光、兩行硬接在一起，重複組
 * 會誤吃到不屬於它的下一行。
 *
 * @param {string[]} lines
