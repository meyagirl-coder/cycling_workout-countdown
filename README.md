...
不知道要貼哪個 event ID 測試的話，部署好之後直接用瀏覽器打開：

```
https://<你的 Vercel 網域>/api/intervals-events
```

會回傳最近課表事件的 JSON 清單（預設抓過去 7 天到未來 30 天），每筆事件的
`id` 就是可以拿去「從 intervals.icu 載入」欄位測試的 event ID。也可以自己帶
`?oldest=YYYY-MM-DD&newest=YYYY-MM-DD` 指定日期範圍。跟 `download.zwo` 那支
proxy 一樣，Athlete ID／API Key 只留在伺服器端，不會出現在回應裡。
