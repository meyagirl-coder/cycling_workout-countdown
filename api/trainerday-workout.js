...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(502).json({ error: '連線 TrainerDay 失敗，請稍後再試，或改用「直接複製貼上文字內容」' });
    return;
  }

  if (!upstreamRes.ok) {
    if (upstreamRes.status === 404) {
      res.status(404).json({ error: '在 TrainerDay 上找不到這份課表，請確認網址是否正確，或改用「直接複製貼上文字內容」' });
      return;
    }
    res.status(502).json({ error: `TrainerDay 回傳錯誤（HTTP ${upstreamRes.status}），請改用「直接複製貼上文字內容」` });
    return;
  }
